...
def main():
    ...
    parser.add_argument(
        "--fasta",
        help="Path to metagenome.fasta to extract corresponding sequences 
from `input`",
        required=True,
    )
    parser.add_argument(
        "--output",
        help="Path to output directory to write refinement bins (Default 
is header_column, i.e. refinement_2)",
    )
    parser.add_argument(
        "--column",
        help="""
    ...
    with open(args.fasta) as fh:
        records = {record: seq for record, seq in fasta_parser(fh) if 
record in contigs}
    ...
    for refined_bin, dff in df.groupby(column):
        ...
        bin_records = [
            (record, seq) for record, seq in records.items() if record in 
bin_contigs
        ]
